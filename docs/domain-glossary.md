...
| Định mức tiết dạy | `TeacherTeachingNorm` / `weeklyNormSessions` | Mục tiêu tiết dạy trung bình mỗi tuần được lấy từ rule snapshot theo cấp trường; THCS/THPT không được hard-code ngoài rule có source/approval. | `weeklyNormBySchoolLevel`, `teachingWeeksForNorm` | Rule codes `RULE-TEACH-002`/`RULE-TEACH-003`; `TeacherLoadCalculation` báo cả weekly average và annual suy ra. **Đã có ở P2.1-T02.** |
| Giảm định mức/kiêm nhiệm | `TeacherWorkloadReduction` / `reductionSessionsPerWeek` | Mức giảm theo chức trách/kiêm nhiệm của đúng giáo viên, chỉ áp dụng khi rule scoped theo actor, có nguồn, hiệu lực và approval. | `roleCode`, `reductionSessionsPerWeek`, `scope.actorId`, source/approval | Rule code prefix `RULE-TEACH-REDUCTION-` trong `RuleSetSnapshot`; không tự suy diễn mức giảm từ role name. **Đã có contract/evaluation ở P2.1-T02.** |
| Báo cáo tải dạy | `TeacherLoadCalculation` / `teacher-loads` | So sánh tải đã phân công theo tuần với target average sau reduction, kèm annual projection, trạng thái và source rule. | `assignedAverageWeeklySessions`, `targetAverageWeeklySessions`, `annualTargetSessions`, `status`, `enforcement`, `ruleSources` | API `GET /schools/:schoolId/academic-periods/:periodId/teacher-loads?ruleSnapshotId=...`; report mặc định `REPORT_ONLY`, không là hard cap. **Đã có ở P2.1-T02.** |
...
| Availability giáo viên | `TeacherAvailabilityRule` / `teacherAvailability` | Khoảng giáo viên không thể dạy hoặc mong muốn tránh, theo ngày, ca, tiết và khoảng hiệu lực. `HARD_UNAVAILABLE` không được xếp; `STRONG_PREFERENCE` và `SOFT_WISH` là chi phí có thể vi phạm. | `teacherId`, `strength`, `weight`, `dayOfWeek`, `shiftCode`, `period`, `blockedSlotIds`, `effectiveFrom`, `source` | `RuleDefinition` trong `RuleSetSnapshot`; API `GET /schools/:schoolId/academic-periods/:periodId/teacher-availability`; NestJS contract và Python `TEACHER-AVAILABILITY-1.0.0`. **Đã có ở P2.1-T03.** |
...
| Phiên bản thời khóa biểu | `ScheduleVersion` / `scheduleVersionId` | Snapshot nghiệp vụ có thể xem, review, phê duyệt, khóa, công bố hoặc lưu trữ; khác với kết quả tạm của một job. Lifecycle chuẩn là `DRAFT → IN_REVIEW → APPROVED → LOCKED → PUBLISHED → ARCHIVED`, trong đó `IN_REVIEW → DRAFT` là đường trả về khi cần sửa. | `id`, `schoolId`, `academicPeriodId`, `versionNumber`, `status`, `sourceRunId`, `createdBy`, `ruleSnapshotId`, các hash snapshot, actor/thời điểm/lý do transition | PostgreSQL `schedule_versions` và append-only `schedule_version_transitions`; NestJS `TimetableModule` sở hữu API transition. Snapshot payload và assignment không được sửa sau `PUBLISHED`; một kỳ học chỉ có một bản `PUBLISHED` hiện hành. **Đã có ở P2.4-T01.** |
...
- Không gọi `weeklyNormSessions` là hard weekly limit nếu snapshot không có
  `hardWeeklyLimitSessions`; `TeacherLoadCalculation` phải hiển thị rõ
  `enforcement: REPORT_ONLY` hay `HARD_CAP`.
...
4. `SoftConstraint` và `Preference` được evaluate/enforce bằng snapshot có
   trọng số, nguồn, ngày hiệu lực. Availability giáo viên đã có contract
   `TEACHER-AVAILABILITY-1.0.0`; các rule khác vẫn phải có version riêng.
5. Các capability chỉnh sửa assignment trước khi publish và chính sách approval/stakeholder sign-off vẫn là task tiếp theo; P2.4-T01 chỉ chốt lifecycle, snapshot provenance, current-plan index và transition audit.
