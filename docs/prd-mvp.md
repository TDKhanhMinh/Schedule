...
| FR-006 | Rule profile phải reference register version, source, effective date, applicability và approval. | AT-01, AT-10 | PARTIAL — versioned profile/definition/snapshot schema, source/effective/scope/approval metadata and hash are implemented; approval workflow and solver enforcement remain follow-up gates. |
...
| FR-009 | Solver phải enforce hard constraints class/teacher, fixed/allowed slots và trả diagnostics khi infeasible. | AT-07, AT-08 | PASS(local) — API/Python pre-solve gate catches necessary-condition failures before CP-SAT; CP-SAT remains final hard-constraint authority; room assignment remains open. |
| FR-010 | Soft constraints/preferences phải có weight, source, rule version và giải thích vi phạm. | AT-09, AT-10 | PARTIAL — versioned availability contract, hard filtering, weighted strong/soft penalties and diagnostics pass locally; approval/pilot and broader rule catalog remain open. |
