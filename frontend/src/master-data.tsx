...
const emptyForm: Record<MasterDataEntity, Record<string, string>> = {
  ...
  assignment: { classId: "", subjectId: "", teacherId: "", roomId: "", requiredSessions: "1" },
};
...
> = {
  school: [
    { key: "code", label: "Mã trường", required: true, placeholder: "THCS-DEMO" },
    { key: "name", label: "Tên trường", required: true, placeholder: "Trường THCS Demo" },
    { key: "timezone", label: "Múi giờ", placeholder: "Asia/Ho_Chi_Minh" },
  ],
  period: [
    { key: "academicYear", label: "Năm học", required: true, placeholder: "2026-2027" },
    { key: "termCode", label: "Mã học kỳ", required: true, placeholder: "TERM_1" },
    { key: "name", label: "Tên khung năm học", required: true, placeholder: "Học kỳ I" },
  ...
  slot: [
    ...
    { key: "shiftCode", label: "Buổi", placeholder: "MORNING" },
    { key: "startsAt", label: "Giờ bắt đầu", type: "time" },
    { key: "endsAt", label: "Giờ kết thúc", type: "time" },
  ],
  teacher: [
    { key: "code", label: "Mã giáo viên", required: true, placeholder: "GV-001" },
    { key: "displayName", label: "Tên giáo viên", required: true, placeholder: "Nguyễn An" },
  ],
  class: [
    { key: "code", label: "Mã lớp", required: true, placeholder: "7A" },
    { key: "name", label: "Tên lớp", required: true, placeholder: "Lớp 7A" },
    { key: "grade", label: "Khối", type: "number", required: true },
  ],
  subject: [
    { key: "code", label: "Mã môn", required: true, placeholder: "MATH" },
    { key: "name", label: "Tên môn", required: true, placeholder: "Toán" },
  ],
  room: [
    { key: "code", label: "Mã phòng", required: true, placeholder: "P-A" },
    { key: "name", label: "Tên phòng", required: true, placeholder: "Phòng A" },
    { key: "roomType", label: "Loại phòng", placeholder: "STANDARD" },
...
export function MasterDataScreen() {
  ...
  const loadBaseData = useCallback(async () => {
    ...
    setError("");
    try {
      const [schoolRows, periodRows, teacherRows, classRows, subjectRows, roomRows] = await Promise.all([
        request<School[]>("/schools"),
        request<AcademicPeriod[]>(`/schools/${frontendConfig.schoolId}/academic-periods`),
        request<Teacher[]>(`/schools/${frontendConfig.schoolId}/teachers`),
        request<SchoolClass[]>(`/schools/${frontendConfig.schoolId}/classes`),
        request<Subject[]>(`/schools/${frontendConfig.schoolId}/subjects`),
        request<Room[]>(`/schools/${frontendConfig.schoolId}/rooms`),
      ]);
  ...
  const loadPeriodData = useCallback(async (periodId: string) => {
    ...
    try {
      const [slotRows, assignmentRows] = await Promise.all([
        request<TimeSlot[]>(`/schools/${frontendConfig.schoolId}/academic-periods/${periodId}/time-slots`),
        request<LessonRequirement[]>(
          `/schools/${frontendConfig.schoolId}/academic-periods/${periodId}/lesson-requirements`,
        ),
      ]);
      setSlots(slotRows);
