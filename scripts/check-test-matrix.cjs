const fs = require("node:fs");
const path = require("node:path");

const root = path.resolve(__dirname, "..");
const matrixPath = path.join(root, "docs", "test-matrix-p2-5-t04.md");
const matrix = fs.readFileSync(matrixPath, "utf8");

const requiredFiles = [
  "backend/src/auth/auth.guard.test.ts",
  "backend/src/imports/imports.service.test.ts",
  "backend/src/contracts/pre-solve.test.ts",
  "backend/src/timetable/schedule-version.service.test.ts",
  "backend/src/timetable/schedule-export.service.test.ts",
  "backend/src/timetable/freeze-scope.test.ts",
  "backend/src/observability/observability.service.test.ts",
  "scripts/run-security-review.mjs",
  "scripts/run-load-soak-benchmark.mjs",
  "scripts/run-disaster-recovery-evidence.mjs",
  "scripts/generate-release-record.mjs",
  "scripts/validate-multi-tenant-design.mjs",
  "scripts/validate-tauri-offline-decision.mjs",
  "backend/src/auth/tenant-scope.test.ts",
  "backend/src/integrations/integration-contract.test.ts",
  "backend/src/timetable/public-schedule.service.test.ts",
  "backend/src/jobs/optimization-queue.service.test.ts",
  "backend/src/worker/optimization-worker.test.ts",
  "backend/solver/tests/test_benchmarks.py",
  "backend/solver/tests/test_benchmark_rubric.py",
  "frontend/scripts/smoke.mjs",
  "scripts/test-p2-5-t04-runtime.mjs",
  "scripts/run-cross-tenant-gate.mjs",
];

for (const relativePath of requiredFiles) {
  if (!fs.existsSync(path.join(root, relativePath))) {
    throw new Error(`Test matrix source is missing: ${relativePath}`);
  }
  if (!matrix.includes(relativePath)) {
    throw new Error(`Test matrix does not reference: ${relativePath}`);
  }
}

for (const id of [
  "UT-01",
  "UT-04",
  "UT-06",
  "UT-11",
  "UT-12",
  "UT-13",
  "IT-03",
  "IT-04",
  "IT-05",
  "IT-06",
  "IT-07",
  "IT-08",
  "IT-09",
  "IT-10",
  "UT-15",
  "UT-16",
  "E2E-03",
  "IT-11",
  "E2E-02",
]) {
  if (!matrix.includes(id)) {
    throw new Error(`Required matrix row is missing: ${id}`);
  }
}

console.log(`Test matrix check passed: ${requiredFiles.length} sources and core workflow rows are present.`);
