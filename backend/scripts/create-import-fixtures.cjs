...
const legacyHeaders = ["class code", "subject code", "teacher code", "required sessions", "room code"];
const legacyRows = [["7A", "Toán", "Nguyễn An", "2", "Phòng A"]];
...
(async () => {
  ...
  await writeWorkbook("legacy.xlsx", legacyHeaders, legacyRows);
  await writeWorkbook(
    "missing-required-column.xlsx",
    ["Mã lớp", "Mã môn", "Số tiết"],
    [[validRows[0][0], validRows[0][1], 2]],
  );
  await writeWorkbook("missing-value.xlsx", headers, [[null, validRows[0][1], validRows[0][2], 2, validRows[0][4]]]);
  await writeWorkbook("wrong-number.xlsx", headers, [
    [validRows[0][0], validRows[0][1], validRows[0][2], "hai", validRows[0][4]],
  ]);
  await writeWorkbook("unknown-master-data.xlsx", headers, [
    [validRows[0][0], validRows[0][1], "XYZ", 2, "UNKNOWN-ROOM"],
  ]);
